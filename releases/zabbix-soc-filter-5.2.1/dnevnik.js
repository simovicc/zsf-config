(function () {
  ...
  function flashMsg(text) {
    ...
    setTimeout(function () {
      ...
      msg.textContent = 'Kopirano u clipboard';
    }, 2500);
  }

  function addPresentFromZabbix() {
    if (!browserAPI || !browserAPI.tabs || !browserAPI.tabs.query) {
      flashMsg('Tabovi nisu dostupni');
      return;
    }
    try {
      browserAPI.tabs.query({ url: ['https://monitor.comtradecloud.com/*', 'http://172.31.35.31/*'] }, function (tabs) {
        if (!tabs || tabs.length === 0) {
          flashMsg('Otvori Zabbix tab pa probaj ponovo');
          return;
        }
        browserAPI.tabs.sendMessage(tabs[0].id, { action: 'captureAllPresent' }, function (resp) {
          const c = (resp && typeof resp.count === 'number') ? resp.count : 0;
          loadLog().then(function (l) {
            log = l;
            applyScheduleAutofill();
            render();
            flashMsg('Prisutnih obrađeno: ' + c);
          });
        });
      });
    } catch (e) {
      flashMsg('Greška pri dodavanju');
    }
  }
  ...
  function init() {
    ...
    const addp = $('add-present');
    if (addp) addp.addEventListener('click', addPresentFromZabbix);
