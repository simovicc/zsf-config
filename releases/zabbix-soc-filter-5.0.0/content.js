(function () {
  ...
  function ensureNotifyButton(row, info) {
    ...
    if (isCtInstance()) {
      const eviBtn = document.createElement('button');
      eviBtn.className = 'soc-evi-btn';
      eviBtn.type = 'button';
      eviBtn.textContent = '+ evidencija';
      eviBtn.title = 'Dodaj ovaj alarm u Evidenciju prijavljenih alarma';
      eviBtn._rowInfo = info;
      eviBtn.addEventListener('mousedown', function (e) { e.stopPropagation(); });
      eviBtn.addEventListener('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        const ri = eviBtn._rowInfo;
        if (ri) {
          captureDutyLog(ri, true);
          eviBtn.textContent = 'dodato';
          eviBtn.classList.add('soc-evi-done');
          setTimeout(function () {
            eviBtn.textContent = '+ evidencija';
            eviBtn.classList.remove('soc-evi-done');
          }, 1500);
          showToast('Dodato u evidenciju: ' + (ri.host || ''), false);
        }
      });
      targetCell.appendChild(eviBtn);
      try { eviBtn.style.right = (btn.offsetWidth + 18) + 'px'; } catch (e) {}
    }
